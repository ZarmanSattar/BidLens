 ...
 *   requirement id.
 * @param {Map<string, object>} [libraryById] content_library rows keyed by id,
 *   used only to turn each covered item's library_entry_ids into titles.
 *   Optional: omit it and coveredItems still carry the ids, with null titles.
 * @returns {object} Counts, the missing REQ numbers, and a per-department
 *   breakdown so a gap can be handed to the team that owns it. coveredItems
 *   additionally carry the drafted `content` and the entries it cited.
 *
 * @example
 * computeCoverage(requirements, skeletons).missing // ['REQ-004', 'REQ-006']
 */
function computeCoverage(requirements, skeletons, libraryById) {
  const rows = Array.isArray(requirements) ? requirements : [];
  const have = skeletons instanceof Map ? skeletons : new Map();
  const library = libraryById instanceof Map ? libraryById : new Map();
  ...
  for (const row of rows) {
    ...
    if (have.has(row.id)) {
      // The skeleton row was already fetched by loadSkeletons and, until now,
      // used only as a presence check — the drafted paragraph was read out of
      // the database on every call and thrown away, so nothing in the UI could
      // show it. Carried onto the covered entry instead.
      const skeleton = have.get(row.id) || {};

      const libraryEntryIds = Array.isArray(skeleton.library_entry_ids)
        ? skeleton.library_entry_ids
        : [];

      covered.push({
        ...entry,
        content: String(skeleton.content || '').trim(),
        libraryEntryIds,
        // Resolved here rather than in the browser: the caller already holds
        // the whole library (it needs it for the staleness rules), so a name
        // lookup costs nothing, and a raw UUID is never rendered.
        libraryEntries: libraryEntryIds.map((id) => {
          const found = library.get(id);

          return {
            id,
            // null title means the entry was deleted. In practice a covered
            // item cannot be in that state — evaluateStaleness marks a
            // skeleton citing a deleted entry as stale, so it never reaches
            // this branch — but the caller may pass no library at all, and a
            // missing name must not render as "undefined".
