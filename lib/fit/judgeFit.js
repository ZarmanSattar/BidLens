 ...
 *   [options.onBatch] Called with each batch's judgments as soon as that batch
 *   succeeds, before the next call is made — the hook that lets a run which
 *   dies part-way keep what it earned. Awaited, and its failures are logged
 *   and swallowed: persistence is best-effort, the judgments are not.
 *
 * @returns {Promise<{judgments: Object<string, object>, stats: object,
 *   error: object|null}>}
 *   `judgments` is keyed by the requirement's 1-BASED POSITION IN THE LIST
 *   THIS FUNCTION WAS GIVEN, not by its row id and not by its position within
 *   a batch. A caller that passes a SUBSET (for example, only the
 *   not-yet-judged requirements) must therefore re-key the result against its
 *   own full list — join on each judgment's `requirementId`, which is carried
 *   for exactly that purpose. Each judgment also carries `reqNumber` so a
...
async function judgeFit(requirements, profile, options = {}) {
  ...
  for (let batchIndex = 0; batchIndex < batches.length; batchIndex += 1) {
    ...
    // Just this batch's judgments, for the onBatch hook below.
    const fresh = [];
    ...
    batch.forEach((requirement, offset) => {
      ...
      const judgment = {
        ...
        // requirementId is what persistence and re-keying join on — a caller
        // that judged a SUBSET cannot use `index` to find the row again.
      ...
      judgments[String(globalIndex)] = judgment;
      fresh.push(judgment);
    });

    // Hand this batch to the caller before starting the next one. The whole
    // point is that a run dying at batch 3 keeps batches 1 and 2, so this is
    // awaited here rather than after the loop.
    //
    // A failing hook must not cost us the judgments: they are already in
    // `judgments` and will still be returned. Persistence is best-effort;
    // the answer is not.
    if (typeof options.onBatch === 'function' && fresh.length > 0) {
      try {
        await options.onBatch(fresh, { batchIndex, batches: batches.length });
      } catch (hookError) {
        console.error(
          `[fit/judge] onBatch hook failed for batch ${batchIndex + 1}:`,
          hookError?.message
        );
      }
    }
