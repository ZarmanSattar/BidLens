...
/**
 * Normalizes one file into `{ filename, pages, pageOffset }`.
 *
...
function toFile(file) {
  ...
  const offset = Number(file?.page_offset);

  return {
    filename: file?.filename || 'Untitled file',
    role: file?.role || 'attachment',
    pages,
    // Absent on the plain `{filename, pages}` shape this function also accepts;
    // 0 makes a file with no recorded offset report its own page numbers, which
    // is the right answer for a package of one.
    pageOffset: Number.isFinite(offset) ? offset : 0,
  };
}

/**
 * A page number within one file → the page the reader counts to in the package.
 *
 * `package_page = page_offset + file_page`, which is the rule rfp_files.page_offset
 * was added to express. §5's contract-risk scan already cites package pages
 * because it reads rfps.pages, the whole package concatenated; reporting a
 * file-local page here would mean the two modules cite the same sentence as two
 * different pages.
 *
 * @param {{pageOffset: number}} file
 * @param {number|null} page 1-based page within the file.
 * @returns {number|null} 1-based page within the package.
 */
function toPackagePage(file, page) {
  return typeof page === 'number' ? file.pageOffset + page : page;
}
    ...
    for (const file of list) {
      ...
      for (const hit of hits) {
        if (!distinct.has(hit.value)) {
          // Rebased here, once, so both `values` below and the conflict
          // entries built from `perFile` cite the same package page. Only the
          // page is touched — `hit.value` is what the comparison keys on and
          // it is carried through untouched.
          distinct.set(hit.value, {
            ...hit,
            page: toPackagePage(file, hit.page),
            occurrences: 1,
          });
        } else {
