...
/**
 * Numbered list markers: "1.", "1.2.3.", "A.", "(1)", "IV.".
 *
 * The space after the marker is optional, because PDF extraction routinely
 * drops it: an <ol><li> that reads "1. The Contractor shall..." on the page
 * comes out of pdf-parse as "1.The Contractor shall...". Requiring the space
 * meant a run of such items never started a new block and the whole list
 * collapsed into one candidate.
 *
 * Two guards keep that relaxation from inventing list items:
 *
 *   - When the space is missing the next character must not be a digit, so a
 *     decimal opening a line — "3.5 seconds of latency", "99.5% uptime",
 *     "5.1 million dollars" — is not read as item 3 followed by "5 seconds".
 *     A bare \s* here matches every one of those.
 *   - The roman-numeral branch still requires the space. Roman markers are the
 *     one form that collides with section cross-references, which routinely
 *     open a wrapped line: "IX.A.6., SWAM..." and "III.B.1." both parse as a
 *     marker plus text once the space is optional, splitting a sentence
 *     mid-clause. Measured against the 44-page reference RFP, allowing it cost
 *     two false splits and bought nothing.
 */
const NUMBERED_PATTERN =
  /^\s*(?:(?:\(?\d+(?:\.\d+)*\)?[.)]|\(?[A-Za-z]\)?[.)])(?:\s+\S|[^\s\d])|\(?[IVXLCDM]+\)?[.)]\s+\S)/;
...
/**
 * Words a Title Case heading is allowed to leave lowercase.
 *
 * "1. Scope of Work" and "3. Terms and Conditions" are Title Case even though
 * "of" and "and" are lowercase — that is the convention, not an exception to
 * it. Anything outside this list must be capitalized for the line to qualify.
 */
...
/** Longest a Title Case heading may run. Beyond this it is a sentence. */
const MAX_TITLE_CASE_WORDS = 8;

/**
 * The marker a numbered/lettered heading opens with, and its trailing space.
 *
 * Identical to the prefix the uppercase path has always used — roman numerals
 * included, space still required. Sharing it is what keeps the two paths
 * agreeing about what counts as a marker.
 */
const HEADING_MARKER_PATTERN =
  /^\s*(?:\(?[IVXLCDM]+\)?[.)]|\(?\d+(?:\.\d+)*\)?[.)]|\(?[A-Z]\)?[.)])\s+/;

/**
 * Whether a phrase is Title Case — every significant word capitalized.
 *
 * This is what separates "3. Evaluation Factors" from "3. The contractor
 * shall provide monitoring": both open with a marker and a capital letter, but
 * only one capitalizes throughout. A single lowercase significant word is
 * enough to reject, which is what keeps ordinary sentences out.
 *
 * @param {string} text The phrase after the marker.
 * @returns {boolean}
 */
function isTitleCasePhrase(text) {
  const words = String(text || '').trim().split(/\s+/).filter(Boolean);

  if (words.length === 0 || words.length > MAX_TITLE_CASE_WORDS) {
    return false;
  }
  ...
  for (const word of words) {
    ...
    if (TITLE_CASE_STOPWORDS.has(bare.toLowerCase())) {
      continue;
    }

    significant += 1;

    if (bare[0] !== bare[0].toUpperCase()) {
      return false;
    }
  }

  // An all-stopword phrase ("1. of the") is not a heading.
 ...
 *
 * Headings out of PDF text are short, often carry a roman/numeric/letter
 * prefix, and are either largely uppercase or Title Case. Requiring shortness
 * plus one of those two shapes keeps ordinary sentences from being mistaken
 * for headings.
 *
 * @param {string} line
...
function detectHeading(line) {
  ...
  // Title Case headings — "3. Evaluation Factors", "1. Scope of Work". These
  // score around 0.2 on uppercaseRatio, so the numbered path above rejects
  // them and the section they introduce ends up with a null section: no
  // location signal, and every line under it that lacks "shall" or "must" is
  // dropped from the candidate list without trace.
  //
  // Added as a THIRD path rather than by lowering the 0.5 threshold. That
  // threshold is what stops a sentence with a few proper nouns in it from
  // reading as a heading, and it still does — this path demands full Title
  // Case instead, which a sentence cannot satisfy.
  const markerMatch = HEADING_MARKER_PATTERN.exec(trimmed);

  const isTitleCaseHeading =
    Boolean(markerMatch) &&
    isTitleCasePhrase(trimmed.slice(markerMatch[0].length));

  if (!isShouted && !isNumberedTitle && !isTitleCaseHeading) {
    return null;
  }
